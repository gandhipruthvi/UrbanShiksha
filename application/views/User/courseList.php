        ...
        <div class="row">
          <div class="col-md-3">
            <div class="right-slide left">
                    ...
                    <div id="collapse<?= $key->categoryID?>" class="panel-collapse collapse" data-parent="#accordion">
                      <div class="panel-body">
                        ...
                        foreach ($key->subCategory as $key1) 
                        {
                          ...
                          <ul class="ml-menu">
                            <li>
                              <a class="subcategoryitem" data-id="<?= $key1->subcategoryID;?>" style="color:black;"><?= $key1->subcategoryName;?></a>
                            </li>
                          </ul>
                          <?php
                        }
              ...
              <div id="showMore">
                <a id="showMoreData" style="margin-left: 8px;color: #4682B4;font-size: 18px"><b><i>Show More</i></b></a>
                <a id="showLessData" style="visibility: hidden;display: none;margin-left: 15px;color: #4682B4;font-size: 18px"><b><i>Show Less</i></b></a>                
              </div> 
              <br>             
              <h3>Price</h3>
              <form method="post" action="<?=site_url('User/SortC/')?>" enctype="multipart/form-data">
                <div class="filter-blcok">
                  <div class="panel panel-default" style="background-color: #f5f5f5;border-color: white;padding-left: 15px;height: 35px;">
...
<script type="text/javascript">
  ...
  $(document).on("click","#checkbox-01",function(){
    loadPagination(0);
  });

  $(document).on("click","#checkbox-02",function(){
    loadPagination(0);
  });
  ...
  function loadPagination(pagno){
    ...
    $.ajax({
      type: "POST",
      data : {searcht : txtSearch,checkF : checkFree,checkP : checkPaid, subID: jssubCategoryID},
      url: '<?= site_url('User/CourseC/paginate/')?>'+pagno,
      success: function(result){
        ...
        for (var data in key)
        {
          ...
          htmlText += '<a href="<?=site_url("User/CourseC/viewCourse/")?>'+encodeURIComponent(btoa(key[data].courseID))+'" class="btn2">view more</a>';
          htmlText += '</div>';
          htmlText += '</div>';
          htmlText += '</div>';
...
<script type="text/javascript">
  ...
  $(document).on("click","#showLessData",function(){
    var categoryList = '';
    $.ajax({
