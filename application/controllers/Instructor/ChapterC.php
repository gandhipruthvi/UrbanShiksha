...
class ChapterC extends CI_Controller
{
	...
	{
		$max=$this->chpm->getMaxChapterNumber($courseID);
		...
		$data = array(
			'courseID'=>$courseID,
			'chapterName'=>$name,
			"number"=>$this->input->post("chapterNo"),
		);

		if($radio=="addNew")
		{
			if($this->input->post("chapterNo")<$max)
			{
				$this->chpm->moveChapter($this->input->post("chapterNo"),$courseID);
			}
	...
	public function editTopic($courseID,$topicID,$chapterID)
	{
		...
		$getName = $this->em->fetchCourse($courseID);
		...
		foreach ($cID as $key) 
		{
	        $temp = array(
	            ...
	            "message" => "Course ".$getName->courseName." has been updated",
	...
	public function fetchChapter($chapterID,$courseID)
	{
		$chapterData=$this->chpm->fetchChapterData($chapterID);
		$chapterNumber=$this->chpm->fetchCourseNumber($courseID);
		$data=array(
	...
	public function editChapter($chapterID,$courseID)
	{
		$preNumber=$this->input->post("previousNumber");
		$newNumber=$this->input->post("newNumber");
		$data=array(
			"chapterName"=>$this->input->post("txtTopicName"),
			"number"=>$this->input->post("newNumber")
		);
		if($newNumber<$preNumber)
		{
			$this->chpm->editChapterAbove($preNumber,$newNumber,$courseID,$data);
		}
		elseif($newNumber>$preNumber)
		{
			$this->chpm->editChapterBelow($preNumber,$newNumber,$courseID,$data);
		}
		$this->chpm->editChapter($chapterID,$data);
